...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    ...
    $delete_stmt = $conn->prepare("DELETE FROM fpoetries WHERE id = ?");
    $delete_stmt->bind_param("i", $id);

    // Execute the SQL statement to delete the record
    if ($delete_stmt->execute() === TRUE) {
        echo '<script>alert("Featured Poetry deleted successfully!"); window.location.href="uploaded-featured-poetry.php";</script>';
    } else {
