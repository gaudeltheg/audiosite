...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT * FROM fpoetries WHERE id = $id";
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        $update_stmt = $conn->prepare("UPDATE fpoetries SET title = ?, audiofile = ? WHERE id = ?");
        $update_stmt->bind_param("ssi", $title, $audiofile, $id);

        // Execute the SQL statement to update the record
        if ($update_stmt->execute() === TRUE) {
            echo '<script>alert("FPoetry updated successfully!"); window.location.href="uploaded-featured-poetry.php";</script>';
        } else {
