...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_stmt = $conn->prepare("SELECT id FROM podcasts WHERE title = ? AND id <> ?");
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        $update_stmt = $conn->prepare("UPDATE podcasts SET title = ?, author = ?, audiofile = ?, image = ?, lyrics = ? WHERE id = ?");
        ...
        if ($update_stmt->execute() === TRUE) {
            echo '<script>alert("Podcast updated successfully!"); window.location.href="uploaded-podcast.php";</script>';
        } else {
