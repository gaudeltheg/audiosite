...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['audio_file']['error'] == UPLOAD_ERR_OK) {
        ...
        $check_stmt = $conn->prepare("SELECT id FROM fpodcasts WHERE title = ?");
        ...
        if ($check_stmt->num_rows > 0) {
            ...
            $insert_stmt = $conn->prepare("INSERT INTO fpodcasts (title, audiofile) VALUES (?, ?)");
            $insert_stmt->bind_param("ss", $audio_title, $audio_file);

            // Check if the file already exists in the destination directory
            $target_dir = "fpodcast/"; // Directory where audio files will be uploaded
...
<div class="container">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
        <label for="audio_title">Featured Podcast Title:</label>
        <input type="text" id="audio_title" name="audio_title" required>

        <label for="audio_file">Podcast File:</label>
