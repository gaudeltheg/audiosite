...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_stmt = $conn->prepare("SELECT id FROM podcasts WHERE title = ?");
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        $insert_stmt = $conn->prepare("INSERT INTO podcasts (title, author, audiofile, image, lyrics) VALUES (?, ?, ?, ?, ?)");
        $insert_stmt->bind_param("sssss", $audio_title, $author_name, $audio_file, $image_file, $lyrics_description);
        ...
        $target_dir = "podcasts/"; // Directory where audio files will be uploaded
