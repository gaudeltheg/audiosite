...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $lyrics = $_POST['editLyrics'];

    // Fetch the existing data from the database
    $query = "SELECT * FROM poetries WHERE id = $id";
    $result = $conn->query($query);
    $row = $result->fetch_assoc();

    // Check if a new image file is uploaded
    $image = $_FILES['editImage']['size'] > 0 ? $_POST['editImage'] : $row['image'];
    ...
    $check_stmt = $conn->prepare("SELECT id FROM poetries WHERE title = ? AND id <> ?");
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        $update_stmt = $conn->prepare("UPDATE poetries SET title = ?, author = ?, audiofile = ?, image = ?, lyrics = ? WHERE id = ?");
        $update_stmt->bind_param("sssssi", $title, $author, $audiofile, $image, $lyrics, $id);

        // Execute the SQL statement to update the record
        if ($update_stmt->execute() === TRUE) {
            echo '<script>alert("Poetry updated successfully!"); window.location.href="uploaded-poetry.php";</script>';
        } else {
