...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT * FROM writings WHERE id = $id";
    $result = $conn->query($query);
    $row = $result->fetch_assoc();

    // Prepare and bind parameters for checking duplicate title
    $check_stmt = $conn->prepare("SELECT id FROM writings WHERE title = ? AND id <> ?");
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        $update_stmt = $conn->prepare("UPDATE writings SET title = ?, author = ?, description = ? WHERE id = ?");
        $update_stmt->bind_param("sssi", $title, $author, $description, $id);

        // Execute the SQL statement to update the record
        if ($update_stmt->execute() === TRUE) {
            echo '<script>alert("Writing updated successfully!"); window.location.href="uploaded-writing.php";</script>';
        } else {
